    ...
    if request.method == 'POST':
        ...
        if user is not None and username != 'admin':
            ...
            with open (__package__+'/data/'+str(request.user)+'.csv','w') as cf:
                cf.write('GrpEntry,BearingType,SubType,Code,Quantity,DateTime,LastAmt,EntryNo,CodeB4Split')
                cf.write('\n')
        ...
        if actionType  == 'add':
            ...
            if grpEntry == 'no':
                with open (__package__+'/data/solo_'+uFileName+'.csv','w') as cfSolo:
                    ...
                    cfSolo.write('\n')
                    cfSolo.write(grpEntry + ',' + optType + ',' + str(oddevenSubType)+str(genericSubType) +',' + str(codeNum) +',' + str(usrDisplayDict['lastEntry'][4]) +',' + DtTm.strftime('%Y-%m-%d %H:%M:%S') + ',' + str(codeVal) + ',' + entryNo+ ','+str(codeB4Split))
            ...
            with open (__package__+'/data/'+uFileName+'.csv','w') as cf:
                ...
                for lCnt, line in enumerate(csvrdr):                        
                    if lCnt == int(entryNo)-1:                                                
                        ...
                        toWrt = grpEntry + ',' + optType + ',' + str(oddevenSubType)+str(genericSubType) +',' + str(codeNum) +',' + str(usrDisplayDict['lastEntry'][4]) +',' + DtTm.strftime('%Y-%m-%d %H:%M:%S') + ',' + str(codeVal) + ',' + entryNo+ ','+codeB4Split
                        cf.write(toWrt)
                        cf.write('\n')                         
                        continue
                    cf.write(line)
